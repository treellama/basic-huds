-- Basic M1 HUD (C) 2024 by Gregory Smith, based on the default Marathon HUD
...
function Triggers.draw()
  ...
  if true then
    ...
    Screen.fill_rect(x - adj(2), y - adj(2), w + adj(2), h + adj(2), InterfaceColors["inventory background"])
  ...
  if #Game.players > 1 then
     ...
     local x = rects["inventory"].x
     local h = int_fonts["player name"].line_height
     local y = rects["inventory"].y - h
     local w = Screen.width - x
...
--  hud_rect.x = math.floor((ww - hud_rect.width) / 2)
  ...
  hud_rect.y = hud_rect.y + hud_rect.height - adj(InterfaceRects["inventory"].y) - adj(InterfaceRects["inventory"].height)
  ...
  if termsize == SizePreferences["double"] then
     if (wh - adj(InterfaceRects["motion sensor"].height) * 2) >= 640 and ww >= 1280 then
        Screen.term_rect.width = 1280
     end
  elseif termsize == SizePreferences["largest"] then
     Screen.term_rect.width = math.min(ww, math.max(640, 2 * (wh - adj(InterfaceRects["motion sensor"].height) * 2)))
  end
  ...
  Screen.world_rect.width = ww
  Screen.world_rect.height = wh
  ...
  rects["motion sensor"].x = 0
  rects["motion sensor"].y = wh - rects["motion sensor"].height

  rects["shield"].x = rects["motion sensor"].x + rects["motion sensor"].width + adj(4)
  rects["shield"].y = wh - rects["shield"].height

  rects["oxygen"].x = rects["shield"].x + rects["shield"].width + adj(4)
  rects["oxygen"].y = wh - rects["oxygen"].height

  Screen.text_margins.bottom = rects["motion sensor"].height + adj(4)
